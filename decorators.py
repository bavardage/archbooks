...
def user_passes_test_ajax(test_func, login_url=None, redirect_field_name=REDIRECT_FIELD_NAME):
    """
...
def login_required_ajax(function=None, redirect_field_name=REDIRECT_FIELD_NAME):
    """
    Decorator for views that checks that the user is logged in, redirecting
    to the log-in page if necessary.
    """
    actual_decorator = user_passes_test_ajax(
...
def permission_required_ajax(perm, login_url=None):
    """
    Decorator for views that checks whether a user has a particular permission
    enabled, redirecting to the log-in page if necessary.
    """
    return user_passes_test_ajax(lambda u: u.has_perm(perm), login_url=login_url)
...
class _CheckLogin(object):
    ...
    def __call__(self, request, *args, **kwargs):
        if self.test_func(request.user):
            ...
            return HttpResponse("ajax auth error")
